...
import type { MarketPnl } from "@structbuild/sdk";
...
type MarketAccessor = (row: MarketPnl) => number | null | undefined;
...
function usdWithCountColumn(
	...
	ctx: SortCtx,
): ColumnDef<MarketPnl, unknown> {
	return {
		id: spec.id,
		meta: { title: spec.title },
		header: sortableHeader<TraderMarketSortBy, MarketPnl>(spec.title, spec.sortKey, ctx),
		size: spec.size,
		cell: ({ row }) => {
...
function buildColumns(ctx: SortCtx): ColumnDef<MarketPnl, unknown>[] {
	const h = (title: string, sortKey?: TraderMarketSortBy) =>
		sortableHeader<TraderMarketSortBy, MarketPnl>(title, sortKey, ctx);
	...
	return [
		...
		numericCol<MarketPnl>({ id: "realizedPnl", title: "Realized PnL", accessor: (r) => r.realized_pnl_usd, size: 128, format: numericFormat.currency, colorizePnl: true, header: h("Realized PnL", "realized_pnl_usd") }),
		numericCol<MarketPnl>({ id: "fees", title: "Fees", accessor: (r) => r.total_fees, size: 112, format: numericFormat.currency, header: h("Fees", "total_fees") }),
		dateCol<MarketPnl>({ id: "lastTradeAt", title: "Last Trade", accessor: (r) => r.last_trade_at, size: 128, header: h("Last Trade", "last_trade_at") }),
		numericCol<MarketPnl>({ id: "unrealizedPnl", title: "Unrealized PnL", accessor: (r) => r.unrealized_pnl_usd, size: 128, format: numericFormat.currency, colorizePnl: true, header: h("Unrealized PnL", "unrealized_pnl_usd") }),
		...
		numericCol<MarketPnl>({ id: "sharesBought", title: "Shares Bought", accessor: (r) => r.total_shares_bought, size: 132, format: numericFormat.count, header: h("Shares Bought", "total_shares_bought") }),
		numericCol<MarketPnl>({ id: "sharesSold", title: "Shares Sold", accessor: (r) => r.total_shares_sold, size: 128, format: numericFormat.count, header: h("Shares Sold", "total_shares_sold") }),
		numericCol<MarketPnl>({ id: "outcomesTraded", title: "Outcomes", accessor: (r) => r.outcomes_traded, size: 104, format: numericFormat.integer, header: h("Outcomes", "outcomes_traded") }),
		dateCol<MarketPnl>({ id: "firstTradeAt", title: "First Trade", accessor: (r) => r.first_trade_at, size: 128, header: h("First Trade", "first_trade_at") }),
	];
}
...
export default function TraderMarkets({ address, page, pageNumber, sortBy, sortDirection, tabs, onRefresh }: Props) {
	const {
		...
		handleSortChange,
		onPageIndexChange,
	} = useTraderBreakdownTable<TraderMarketSortBy, MarketPnl>({
		address,
		page,
		pageNumber,
		sortBy,
